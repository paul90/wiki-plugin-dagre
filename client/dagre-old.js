(function () {
  ...
  emit = async function ($item, item) {
    // load the dagre libraries, but first d3
    var d3URL = wiki.pluginRoutes['dagre'] + '/client/js/d3/d3.v5.min.js'
  ...
  bind = async function ($item, item) {
    ...
    try {
      ...
      $item.html(message('Error reading dot'))
    }
