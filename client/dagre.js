(function () {
  ...
  async function makedot ($item, item) {
    ...
    if (m = text.match(/^DOT ((strict )?(di)?graph)\n/)) {
      ...
      var dot = await evalDSL(root, context, [])
      console.log('dot', dot)
    ...
    async function evalDSL(tree, context, dot) {
      let deeper = []
      var pc = 0
      while (pc < tree.length) {
        ...
        if (Array.isArray(ir)) {
          ...
          console.log('evalDSL', ir.toString())
          dot.push(ir)
        }
      }
      ...
      for (var i=0; i<deeper.length; i++) {
        let child = deeper[i]
        await evalDSL(child.tree, child.context, dot)
      }
  ...
  bind = async function($item, item) {
    ...
    $item.click((e) => {
      ...
      let svg = $item.find('dagre-viewer').get(0).shadowRoot.querySelector('svg').outerHTML
    ...
    try {
      dot = await makedot($item, item)
    } catch (err) {
      console.log('makedot', err)
      $item.html(message(err.message))
    }

    $item.find('.viewer').html(`<dagre-viewer>${dot}</dagre-viewer>`)
    let $viewer = $item.find('dagre-viewer')
    $viewer.dblclick(event => {
      if (event.shiftKey) {
        event.stopPropagation()
        let svg = $item.find('dagre-viewer').get(0)
          .shadowRoot.querySelector('svg').cloneNode(true)
        wiki.dialog('Dagre Viewer', svg)
      }
    })
    $viewer.get(0).render()
  }
  ...
  if (typeof window !== "undefined" && window !== null) {
    moduleLoaded = wiki.getModule(wiki.pluginRoutes['dagre'] + '/client/dagre-viewer.js')
    window.plugins.dagre = {emit, bind};
  }
